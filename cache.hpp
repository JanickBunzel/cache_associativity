...
SC_MODULE(Cache) {
    ...
    unsigned int lines_per_set = 4;
    int num_sets = cache_lines / lines_per_set;

    // Numbers of bits represented in an address:
    // - offset_bits: Bits for the position of a block in a cache line
    unsigned offset_bits;
    // - index_bits: Bits for the position of a set in the cache
    unsigned index_bits;
    // - tag_bits: Bits for the tag of a cache line
    unsigned tag_bits;
    ...
    */
    Cache (sc_module_name name, int directMapped, unsigned chacheLines, unsigned chacheLineSize, unsigned cacheLatency, unsigned memoryLatency)
        : sc_module(name) , direct_mapped(directMapped), cache_lines(chacheLines), cache_line_size(chacheLineSize), cache_latency(cacheLatency), memory_latency(memoryLatency)
    {
